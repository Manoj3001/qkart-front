...
export const getTotalCartValue = (items = []) => {
  return items.reduce((acc, curr) => { // console.log(curr.cost, curr.qty); 
...
export const getTotalItems = (items = []) => { 
  let itemsCount = items.map((item) => { 
    let count = 0; if (item.productId) count++; return count; });
 ...
 */
const ItemQuantity = ({
  value, handleAdd, handleDelete, isReadOnly=false, }) => {
     return ( 
  ...
  return (
    <>
      <Box className="cart">
        ...
        padding="1rem" key={item.productId}> 
        <Box className="image-container"> <img // Add product 
        image src={item.image} // Add product name as alt eext 
        ...
        {isReadOnly ? null :(<Box display="flex" justifyContent="flex-end" className="cart-footer">
          <Button
            ...
            onClick={() => { history.push("/checkout"); window.scrollTo(0, 0);}}
          >
            Checkout
          </Button>
        </Box>
